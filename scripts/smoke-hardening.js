...
function checkTenantMenuGalleryAdmin() {
  ...
  assertIncludes(tenantAdmin, 'Assisted ordering, automatic vendor sending, payment reconciliation, external logging/SIEM, alerting, compliance exports, advanced analytics, forecasting, BI exports, scheduled reports, advanced kitchen automation, automatic inventory consumption, advanced staff workflows, billing, domains, email, and WhatsApp automation remain future tenant admin work.', 'Tenant admin dashboard future module boundary');
  ...
  assertIncludes(readme, 'Restaurant staff access includes tenant-scoped menu, gallery, profile, settings, staff management foundation, reservations management, table management foundation, order status management foundation, kitchen queue operations, inventory management foundation, recipe/ingredient linkage foundation, recipe consumption preview, supplier records, manual purchase requests, purchase invoice recording, manual purchase invoice payment records, payment settings foundation, payment provider event tracking foundation, read-only operations reports, tenant audit logs, public tenant order creation when ONLINE_ORDERING is enabled, tenant table QR ordering when TABLE_QR_ORDERING is enabled, tenant public order support actions, and tenant public reservation support actions; assisted ordering, real payment processing, refunds, messaging, advanced guest automation, advanced kitchen automation, automatic inventory consumption, automatic recipe depletion, invoice automation, external logging/SIEM, alerting, compliance exports, production secrets rotation, advanced analytics, forecasting, BI exports, scheduled reports, billing, domains, email, and WhatsApp automation remain future work.', 'README Batch 75 updated tenant staff access note');
...
function checkTenantProfileSettingsAdmin() {
  ...
  assertIncludes(readme, 'Feature/module activation beyond online/table ordering, assisted ordering, payments, advanced kitchen automation, inventory consumption, automatic recipe depletion, broader staff management, billing, domains, email, and WhatsApp automation remain future work.', 'README Batch 58 future boundary note');
...
function checkTenantReservationsManagement() {
  ...
  assertIncludes(readme, 'Restaurant staff access includes tenant-scoped menu, gallery, profile, settings, staff management foundation, reservations management, table management foundation, order status management foundation, kitchen queue operations, inventory management foundation, recipe/ingredient linkage foundation, recipe consumption preview, supplier records, manual purchase requests, purchase invoice recording, manual purchase invoice payment records, payment settings foundation, payment provider event tracking foundation, read-only operations reports, tenant audit logs, public tenant order creation when ONLINE_ORDERING is enabled, tenant table QR ordering when TABLE_QR_ORDERING is enabled, tenant public order support actions, and tenant public reservation support actions; assisted ordering, real payment processing, refunds, messaging, advanced guest automation, advanced kitchen automation, automatic inventory consumption, automatic recipe depletion, invoice automation, external logging/SIEM, alerting, compliance exports, production secrets rotation, advanced analytics, forecasting, BI exports, scheduled reports, billing, domains, email, and WhatsApp automation remain future work.', 'README Batch 75 tenant staff access note');
...
function checkTenantKitchenQueueOperations() {
  ...
  assertIncludes(tenantKitchenClient, 'Recipe previews are read-only; no stock is deducted and no payment, messaging, inventory, or recipe workflow is triggered.', 'Tenant kitchen client boundary copy');
  ...
  assertIncludes(blocker, 'automatic recipe depletion, order inventory consumption, automatic supplier sending, invoice automation, staff invitations, self-service password reset flows, external logging/SIEM, alerting, compliance exports, production secrets rotation, advanced analytics, forecasting, BI exports, scheduled reports, billing, domains, CRM, payroll, email, and WhatsApp automation remain future work.', 'Tenant admin doc future work boundary');
...
function checkTenantInventoryManagementFoundation() {
  ...
  const inventoryFeatureSource = `${tenantInventoryPage}\n${tenantInventoryClient}`;
  ...
  assertNotIncludes(inventoryFeatureSource, 'Supplier', 'Tenant inventory page must not embed supplier controls');
  assertNotIncludes(inventoryFeatureSource, 'Invoice', 'Tenant inventory page must not add invoice controls');
  ...
  assertIncludes(blocker, 'Assisted ordering, payments, refunds, messaging, advanced guest automation, advanced kitchen automation, automatic recipe depletion, order inventory consumption, automatic supplier sending, invoice automation, staff invitations, self-service password reset flows, external logging/SIEM, alerting, compliance exports, production secrets rotation, advanced analytics, forecasting, BI exports, scheduled reports, billing, domains, CRM, payroll, email, and WhatsApp automation remain future work.', 'Tenant admin doc inventory future boundary');
...
function checkTenantRecipeIngredientLinkageFoundation() {
  const schema = read('prisma/schema.prisma');
  const readme = read('README.md');
  const blocker = read('docs/TENANT_ADMIN_ACCESS_FOUNDATION_BLOCKER.md');
  const migrationDirs = fs.readdirSync(path.join(root, 'prisma/migrations'));
  const recipesRoutePath = path.join(root, 'src/app/api/restaurant-admin/recipes/route.js');
  const recipeRoutePath = path.join(root, 'src/app/api/restaurant-admin/recipes/[id]/route.js');
  const ingredientRoutePath = path.join(root, 'src/app/api/restaurant-admin/recipes/[id]/ingredients/route.js');
  const tenantRecipesPagePath = path.join(root, 'src/app/r/[restaurantSlug]/admin/recipes/page.js');
  const tenantRecipesClientPath = path.join(root, 'src/app/r/[restaurantSlug]/admin/recipes/TenantRecipesClient.jsx');

  assert(fs.existsSync(recipesRoutePath), 'Tenant recipes collection API route is missing');
  assert(fs.existsSync(recipeRoutePath), 'Tenant recipe item API route is missing');
  assert(fs.existsSync(ingredientRoutePath), 'Tenant recipe ingredients API route is missing');
  assert(fs.existsSync(tenantRecipesPagePath), 'Tenant recipes admin page is missing');
  assert(fs.existsSync(tenantRecipesClientPath), 'Tenant recipes admin client is missing');

  const recipesRoute = read('src/app/api/restaurant-admin/recipes/route.js');
  const recipeRoute = read('src/app/api/restaurant-admin/recipes/[id]/route.js');
  const ingredientRoute = read('src/app/api/restaurant-admin/recipes/[id]/ingredients/route.js');
  const tenantRecipesPage = read('src/app/r/[restaurantSlug]/admin/recipes/page.js');
  const tenantRecipesClient = read('src/app/r/[restaurantSlug]/admin/recipes/TenantRecipesClient.jsx');
  const tenantNav = read('src/app/r/[restaurantSlug]/admin/TenantAdminNav.jsx');
  const tenantAdmin = read('src/app/r/[restaurantSlug]/admin/page.js');
  const recipeHelper = read('src/lib/recipes.js');
  const recipeApiSource = `${recipesRoute}\n${recipeRoute}\n${ingredientRoute}`;
  const recipeUiSource = `${tenantRecipesPage}\n${tenantRecipesClient}\n${tenantNav}\n${tenantAdmin}`;

  assertIncludes(schema, 'model MenuItemIngredient', 'MenuItemIngredient schema support');
  assertIncludes(schema, 'menuItemId      String', 'MenuItemIngredient menu item linkage');
  assertIncludes(schema, 'inventoryItemId String', 'MenuItemIngredient inventory item linkage');
  assertIncludes(schema, 'restaurantId    String?', 'MenuItemIngredient nullable restaurantId');
  assertIncludes(schema, '@@unique([menuItemId, inventoryItemId])', 'MenuItemIngredient unique menu/inventory mapping');
  assertIncludes(schema, '@@index([restaurantId])', 'MenuItemIngredient restaurantId index');

  assertIncludes(recipesRoute, 'requireRestaurantStaffAccess(request, restaurantSlug)', 'Tenant recipes GET uses restaurant staff auth');
  assertIncludes(recipesRoute, 'requireRestaurantStaffAccess(request, parsed.data.restaurantSlug, { write: true })', 'Tenant recipes POST requires OWNER/MANAGER');
  assertIncludes(recipeRoute, 'requireRestaurantStaffAccess(request, restaurantSlug)', 'Tenant recipe GET uses restaurant staff auth');
  assertIncludes(recipeRoute, 'requireRestaurantStaffAccess(request, parsed.data.restaurantSlug, { write: true })', 'Tenant recipe PUT requires OWNER/MANAGER');
  assertIncludes(ingredientRoute, 'requireRestaurantStaffAccess(request, restaurantSlug)', 'Tenant recipe ingredient GET uses restaurant staff auth');
  assertIncludes(ingredientRoute, 'requireRestaurantStaffAccess(request, parsed.data.restaurantSlug, { write: true })', 'Tenant recipe ingredient writes require OWNER/MANAGER');

  assertIncludes(recipesRoute, 'where: { restaurantId: staff.restaurantId }', 'Tenant recipe menu item list scopes by restaurantId');
  assertIncludes(recipesRoute, 'where: { restaurantId: staff.restaurantId, isActive: true }', 'Tenant recipe inventory selector scopes by restaurantId');
  assertIncludes(recipeRoute, 'where: { id: menuItemId, restaurantId }', 'Tenant recipe helper scopes menu item by restaurantId');
  assertIncludes(recipeRoute, 'where: { id: params.id, restaurantId: staff.restaurantId }', 'Tenant recipe PUT validates menu item ownership');
  assertIncludes(recipeRoute, 'id: { in: uniqueIds }', 'Tenant recipe PUT validates inventory ids');
  assertIncludes(recipeRoute, 'restaurantId,', 'Tenant recipe PUT inventory validation includes restaurantId');
  assertIncludes(ingredientRoute, 'where: { id: menuItemId, restaurantId }', 'Tenant recipe ingredient validates menu item ownership');
  assertIncludes(ingredientRoute, 'where: { id: inventoryItemId, restaurantId, isActive: true }', 'Tenant recipe ingredient validates inventory ownership');
  assertIncludes(ingredientRoute, 'menuItemId: params.id', 'Tenant recipe ingredient scopes by menu item');
  assertIncludes(ingredientRoute, 'restaurantId: staff.restaurantId', 'Tenant recipe ingredient scopes by staff restaurant');

  assertIncludes(recipesRoute, 'validateRecipeIngredientQuantity', 'Tenant recipes validate positive quantity');
  assertIncludes(recipeRoute, 'validateRecipeIngredientQuantity', 'Tenant recipe item validates positive quantity');
  assertIncludes(ingredientRoute, 'validateRecipeIngredientQuantity', 'Tenant recipe ingredient validates positive quantity');
  assertIncludes(ingredientRoute, 'prisma.menuItemIngredient.updateMany', 'Tenant recipe ingredient update uses scoped updateMany');
  assertIncludes(ingredientRoute, 'updateResult.count !== 1', 'Tenant recipe ingredient update checks scoped count');
  assertIncludes(ingredientRoute, 'prisma.menuItemIngredient.deleteMany', 'Tenant recipe ingredient removal uses scoped deleteMany');
  assertIncludes(recipeRoute, 'tx.menuItemIngredient.deleteMany', 'Tenant recipe replacement uses scoped deleteMany');
  assertIncludes(recipeRoute, 'tx.menuItemIngredient.createMany', 'Tenant recipe replacement creates scoped ingredient lines');
  assertIncludes(recipesRoute, 'restaurantId: staff.restaurantId', 'Tenant recipes create writes restaurantId');
  assertIncludes(recipeRoute, 'restaurantId: staff.restaurantId', 'Tenant recipe replacement writes restaurantId');

  assertNotIncludes(recipeApiSource, 'requireAdmin', 'Tenant recipe APIs must not use platform requireAdmin');
  assertNotIncludes(recipeApiSource, 'getAdminFromRequest', 'Tenant recipe APIs must not use platform admin session');
  assertNotIncludes(recipeApiSource, 'prisma.adminUser', 'Tenant recipe APIs must not touch AdminUser');
  assertNotIncludes(recipeApiSource, 'prisma.gatewayLead', 'Tenant recipe APIs must not touch GatewayLead');
  assertNotIncludes(recipeApiSource, 'prisma.order', 'Tenant recipe APIs must not touch orders');
  assertNotIncludes(recipeApiSource, 'orderRecipeConsumption', 'Tenant recipe APIs must not create recipe consumption');
  assertNotIncludes(recipeApiSource, 'inventoryMovement', 'Tenant recipe APIs must not create inventory movements');
  assertNotIncludes(recipeApiSource, 'currentStock: { decrement', 'Tenant recipe APIs must not deplete stock');
  assertNotIncludes(recipeApiSource, 'supplier', 'Tenant recipe APIs must not add supplier automation');
  assertNotIncludes(recipeApiSource, 'invoice', 'Tenant recipe APIs must not add invoice automation');
  assertNotIncludes(recipeApiSource, 'stripe', 'Tenant recipe APIs must not add payment logic');
  assertNotIncludes(recipeApiSource, 'sendWhatsApp', 'Tenant recipe APIs must not send WhatsApp');
  assertNotIncludes(recipeApiSource, 'sendMail', 'Tenant recipe APIs must not send email');
  assertNotIncludes(recipeApiSource, 'billing', 'Tenant recipe APIs must not add billing logic');
  assertNotIncludes(recipeApiSource, 'domain', 'Tenant recipe APIs must not add domain logic');
  assertNotIncludes(recipeApiSource, 'crm', 'Tenant recipe APIs must not add CRM logic');
  assertNotIncludes(recipeApiSource, 'payroll', 'Tenant recipe APIs must not add payroll logic');
  assertNotIncludes(recipeApiSource, 'analytics', 'Tenant recipe APIs must not add analytics engine logic');
  assertNotIncludes(recipeApiSource, 'prisma.menuItemIngredient.update({', 'Tenant recipe APIs must not mutate ingredient by id only');
  assertNotIncludes(recipeApiSource, 'prisma.menuItemIngredient.delete({', 'Tenant recipe APIs must not delete ingredient by id only');

  assertIncludes(recipeHelper, 'normalizeMenuItemRecipe', 'Recipe helper normalizes menu item recipes');
  assertIncludes(recipeHelper, 'getRecipeIngredientEstimatedCost', 'Recipe helper estimates ingredient cost');
  assertIncludes(recipeHelper, 'lowStockIngredientCount', 'Recipe helper exposes low-stock ingredient count');

  assertIncludes(tenantRecipesPage, 'requireRestaurantStaffAccess(cookies(), params.restaurantSlug)', 'Tenant recipes page uses DB-backed staff access');
  assertIncludes(tenantRecipesPage, 'redirect(`/r/${params.restaurantSlug}/admin/login`)', 'Tenant recipes page redirects failed staff validation');
  assertIncludes(tenantRecipesPage, '<TenantRecipesClient', 'Tenant recipes page renders client');
  assertIncludes(tenantRecipesClient, '/api/restaurant-admin/recipes', 'Tenant recipes client uses recipes API');
  assertIncludes(tenantRecipesClient, '/ingredients', 'Tenant recipes client uses ingredient API route');
  assertIncludes(tenantRecipesClient, "'POST'", 'Tenant recipes client creates ingredient links');
  assertIncludes(tenantRecipesClient, "'PUT'", 'Tenant recipes client updates ingredient links');
  assertIncludes(tenantRecipesClient, "method: 'DELETE'", 'Tenant recipes client removes ingredient links');
  assertIncludes(tenantRecipesClient, 'SUPPORT access is read-only', 'Tenant recipes client support read-only state');
  assertIncludes(tenantRecipesClient, 'OWNER or MANAGER access is required', 'Tenant recipes client write role copy');
  assertIncludes(tenantRecipesClient, 'Select tenant inventory item', 'Tenant recipes client inventory item selector');
  assertIncludes(tenantRecipesClient, 'Estimated recipe cost', 'Tenant recipes client estimated recipe cost');
  assertIncludes(tenantRecipesClient, 'linked ingredients need stock attention', 'Tenant recipes client low-stock hint');
  assertIncludes(tenantRecipesClient, 'No ingredients mapped yet', 'Tenant recipes client empty state');
  assertIncludes(tenantRecipesClient, 'Refresh', 'Tenant recipes client manual refresh');
  assertIncludes(tenantRecipesClient, 'window.confirm', 'Tenant recipes client confirms ingredient removal');
  assertNotIncludes(recipeUiSource, '/platform-admin', 'Tenant recipes UI must not link to platform admin');
  assertIncludes(tenantNav, "label: 'Recipes'", 'Tenant admin navigation includes Recipes');
  assertIncludes(tenantNav, "href: `/r/${restaurantSlug}/admin/recipes`", 'Tenant admin navigation recipes route');
  assertIncludes(tenantAdmin, 'recipeCounters', 'Tenant admin dashboard includes recipe counters');
  assertIncludes(tenantAdmin, 'Open recipes', 'Tenant admin dashboard links to recipes');
  assertIncludes(tenantAdmin, 'No automatic stock depletion or order consumption is triggered.', 'Tenant admin dashboard recipe boundary copy');
  assertIncludes(tenantAdmin, 'restaurantId: staff.restaurantId', 'Tenant admin recipe counters scope by restaurantId');

  assertIncludes(readme, 'Tenant recipe and ingredient linkage foundation added.', 'README Batch 63 note');
  assertIncludes(readme, 'Restaurant staff can map tenant menu items to tenant inventory ingredients under `/r/[restaurantSlug]/admin/recipes`.', 'README Batch 63 scope note');
  assertIncludes(readme, 'Estimated recipe cost and linked ingredient stock visibility are informational only.', 'README Batch 63 cost visibility note');
  assertIncludes(readme, 'Automatic recipe depletion, order inventory consumption, inventory movement creation from recipes, automatic supplier sending, invoice automation, payments, billing, domains, CRM, payroll, analytics, email, and WhatsApp automation remain future work.', 'README Batch 63 boundary note');
  assertIncludes(blocker, 'Batch 63 adds tenant-scoped recipe/ingredient linkage between menu items and inventory items.', 'Tenant admin doc Batch 63 note');
  assertIncludes(blocker, 'Tenant recipe reads, ingredient mapping updates, menu item validation, and inventory item validation are scoped by restaurantId.', 'Tenant admin doc recipe scoping boundary');
  assertIncludes(blocker, 'Tenant recipe linkage exposes estimated cost and linked stock visibility only; it does not deduct stock or create inventory movements.', 'Tenant admin doc recipe no-depletion boundary');

  assert(!fs.existsSync(path.join(root, 'src/app/api/billing')), 'Tenant recipe batch should not add billing API route');
  assert(!fs.existsSync(path.join(root, 'src/app/api/payments')), 'Tenant recipe batch should not add payments API route');
  assert(!fs.existsSync(path.join(root, 'src/app/api/email')), 'Tenant recipe batch should not add email API route');
  assert(!fs.existsSync(path.join(root, 'src/app/api/whatsapp')), 'Tenant recipe batch should not add WhatsApp API route');
  assert(!migrationDirs.some((migrationDir) => /tenant.recipe|recipe.linkage|tenant.*ingredient/i.test(migrationDir)), 'Batch 63 should not add a Prisma migration');
}

function checkTenantRecipeConsumptionPreview() {
  const readme = read('README.md');
  const blocker = read('docs/TENANT_ADMIN_ACCESS_FOUNDATION_BLOCKER.md');
  const migrationDirs = fs.readdirSync(path.join(root, 'prisma/migrations'));
  ...
  assert(fs.existsSync(previewRoutePath), 'Tenant recipe consumption preview API route is missing');
  assert(fs.existsSync(tenantKitchenClientPath), 'Tenant kitchen client is missing for recipe preview');
  ...
  const recipeHelper = read('src/lib/recipes.js');
  const previewSource = `${previewRoute}\n${kitchenClient}`;
  ...
  assertIncludes(previewRoute, 'status: order.status', 'Tenant recipe preview returns order status');
  assertNotIncludes(previewRoute, 'restaurantId: order.restaurantId', 'Tenant recipe preview response must not expose restaurantId');
  assertNotIncludes(previewRoute, 'restaurantId: staff.restaurantId,\n      staffRole', 'Tenant recipe preview response must not expose restaurantId');
  assertNotIncludes(previewRoute, 'inventoryItem.update', 'Tenant recipe preview must not update inventory items');
  assertNotIncludes(previewRoute, 'inventoryItem.updateMany', 'Tenant recipe preview must not update inventory items with updateMany');
  assertNotIncludes(previewRoute, 'inventoryMovement.create', 'Tenant recipe preview must not create inventory movements');
  assertNotIncludes(previewRoute, 'order.update', 'Tenant recipe preview must not update orders');
  assertNotIncludes(previewRoute, 'order.updateMany', 'Tenant recipe preview must not update orders with updateMany');
  assertNotIncludes(previewRoute, 'orderRecipeConsumption.create', 'Tenant recipe preview must not create recipe consumption records');
  assertNotIncludes(previewRoute, 'requireAdmin', 'Tenant recipe preview must not use platform requireAdmin');
  assertNotIncludes(previewRoute, 'prisma.adminUser', 'Tenant recipe preview must not touch AdminUser');
  assertNotIncludes(previewRoute, 'prisma.gatewayLead', 'Tenant recipe preview must not touch GatewayLead');
  assertNotIncludes(previewRoute, 'supplier', 'Tenant recipe preview must not add supplier automation');
  assertNotIncludes(previewRoute, 'invoice', 'Tenant recipe preview must not add invoice automation');
  assertNotIncludes(previewRoute, 'stripe', 'Tenant recipe preview must not add payment logic');
  assertNotIncludes(previewRoute, 'sendWhatsApp', 'Tenant recipe preview must not send WhatsApp');
  assertNotIncludes(previewRoute, 'sendMail', 'Tenant recipe preview must not send email');
  assertNotIncludes(previewRoute, 'billing', 'Tenant recipe preview must not add billing logic');
  assertNotIncludes(previewRoute, 'domain', 'Tenant recipe preview must not add domain logic');
  assertNotIncludes(previewRoute, 'crm', 'Tenant recipe preview must not add CRM logic');
  assertNotIncludes(previewRoute, 'payroll', 'Tenant recipe preview must not add payroll logic');
  assertNotIncludes(previewRoute, 'analytics', 'Tenant recipe preview must not add analytics logic');

  assertIncludes(recipeHelper, 'isLowStock', 'Recipe consumption line includes low-stock flag');
  assertIncludes(recipeHelper, 'isOutOfStock', 'Recipe consumption line includes out-of-stock flag');
  assertIncludes(kitchenClient, '/api/restaurant-admin/recipes/preview', 'Tenant kitchen client calls recipe preview API');
  assertIncludes(kitchenClient, 'loadRecipePreview(order)', 'Tenant kitchen client can open recipe preview');
  assertIncludes(kitchenClient, 'Recipe preview', 'Tenant kitchen client shows recipe preview action');
  assertIncludes(kitchenClient, 'Preview only; no stock is deducted', 'Tenant kitchen client no stock deducted copy');
  assertIncludes(kitchenClient, 'Missing recipe mappings', 'Tenant kitchen client displays missing mappings');
  assertIncludes(kitchenClient, 'Current:', 'Tenant kitchen client displays current stock');
  assertIncludes(kitchenClient, 'getStockBadgeClass(line)', 'Tenant kitchen client displays stock warning badges');
  assertIncludes(kitchenClient, 'SUPPORT access is read-only', 'Tenant kitchen client keeps SUPPORT read-only state');
  assertNotIncludes(kitchenClient, 'if (!writable) return;\n    setPreviewLoadingId', 'Tenant kitchen recipe preview must be visible to SUPPORT');
  assertNotIncludes(previewSource, '/platform-admin', 'Tenant recipe preview UI/API must not link to platform admin');

  assertIncludes(readme, 'Tenant recipe consumption preview added.', 'README Batch 64 note');
  assertIncludes(readme, 'Restaurant staff can open read-only ingredient requirements from the tenant kitchen queue.', 'README Batch 64 kitchen preview note');
  assertIncludes(readme, 'Preview shows required quantities, missing recipe mappings, current stock, and low/out-of-stock flags.', 'README Batch 64 preview data note');
  assertIncludes(readme, 'No inventory stock mutation, inventory movement creation, automatic depletion, automatic supplier sending, invoice automation, payments, billing, domains, CRM, payroll, analytics, email, or WhatsApp automation was added.', 'README Batch 64 boundary note');
  assertIncludes(blocker, 'Batch 64 adds tenant-scoped read-only recipe consumption preview for orders.', 'Tenant admin doc Batch 64 note');
  assertIncludes(blocker, 'Tenant recipe consumption preview is read-only, scoped by restaurantId, and shows required ingredient quantities, missing mappings, and current stock warnings.', 'Tenant admin doc Batch 64 boundary');

  assert(!fs.existsSync(path.join(root, 'src/app/api/billing')), 'Tenant recipe preview batch should not add billing API route');
  assert(!fs.existsSync(path.join(root, 'src/app/api/payments')), 'Tenant recipe preview batch should not add payments API route');
...
function checkTenantManualRecipeConsumptionApply() {
  ...
  assertIncludes(readme, 'Automatic consumption on order creation/status changes, automatic supplier sending, invoice automation, payments, billing, domains, CRM, payroll, analytics, email, and WhatsApp automation remain future work.', 'README Batch 65 boundary note');
...
function checkTenantSupplierPurchaseRequestFoundation() {
  const schema = read('prisma/schema.prisma');
  const readme = read('README.md');
  const blocker = read('docs/TENANT_ADMIN_ACCESS_FOUNDATION_BLOCKER.md');
  const migrationPath = path.join(root, 'prisma/migrations/20260611103000_add_supplier_purchase_requests/migration.sql');
  const supplierRoutePath = path.join(root, 'src/app/api/restaurant-admin/suppliers/route.js');
  const supplierItemRoutePath = path.join(root, 'src/app/api/restaurant-admin/suppliers/[id]/route.js');
  const requestRoutePath = path.join(root, 'src/app/api/restaurant-admin/purchase-requests/route.js');
  const requestItemRoutePath = path.join(root, 'src/app/api/restaurant-admin/purchase-requests/[id]/route.js');
  const suppliersPagePath = path.join(root, 'src/app/r/[restaurantSlug]/admin/suppliers/page.js');
  const suppliersClientPath = path.join(root, 'src/app/r/[restaurantSlug]/admin/suppliers/TenantSuppliersClient.jsx');
  ...
  assert(fs.existsSync(migrationPath), 'Batch 66 supplier/purchase request migration is missing');
  assert(fs.existsSync(supplierRoutePath), 'Tenant supplier collection API route is missing');
  assert(fs.existsSync(supplierItemRoutePath), 'Tenant supplier item API route is missing');
  assert(fs.existsSync(requestRoutePath), 'Tenant purchase request collection API route is missing');
  assert(fs.existsSync(requestItemRoutePath), 'Tenant purchase request item API route is missing');
  assert(fs.existsSync(suppliersPagePath), 'Tenant suppliers admin page is missing');
  assert(fs.existsSync(suppliersClientPath), 'Tenant suppliers admin client is missing');
  assert(fs.existsSync(requestsPagePath), 'Tenant purchase requests admin page is missing');
  assert(fs.existsSync(requestsClientPath), 'Tenant purchase requests admin client is missing');

  const migration = read('prisma/migrations/20260611103000_add_supplier_purchase_requests/migration.sql');
  ...
  const suppliersPage = read('src/app/r/[restaurantSlug]/admin/suppliers/page.js');
  const suppliersClient = read('src/app/r/[restaurantSlug]/admin/suppliers/TenantSuppliersClient.jsx');
  ...
  const tenantNav = read('src/app/r/[restaurantSlug]/admin/TenantAdminNav.jsx');
  const tenantAdmin = read('src/app/r/[restaurantSlug]/admin/page.js');
  const supplierApiSource = `${supplierRoute}\n${supplierItemRoute}`;
  ...
  assertIncludes(schema, 'model PurchaseRequestLine', 'PurchaseRequestLine Prisma model');
  ...
  assertIncludes(supplierRoute, 'requireRestaurantStaffAccess(request, parsed.data.restaurantSlug, { write: true })', 'Tenant supplier POST requires OWNER/MANAGER');
  assertIncludes(supplierRoute, 'const where = { restaurantId: staff.restaurantId }', 'Tenant supplier GET scopes by restaurantId');
  assertIncludes(supplierRoute, 'restaurantId: staff.restaurantId', 'Tenant supplier create stamps restaurantId');
  assertIncludes(supplierItemRoute, 'where: { id: params.id, restaurantId: staff.restaurantId }', 'Tenant supplier item route scopes by restaurantId');
  assertIncludes(supplierItemRoute, 'prisma.supplier.updateMany', 'Tenant supplier updates use scoped updateMany');
  assertIncludes(supplierItemRoute, 'updated.count !== 1', 'Tenant supplier updates check scoped update count');
  assertNotIncludes(supplierApiSource, 'prisma.supplier.update({', 'Tenant supplier APIs must not mutate by id only');
  assertNotIncludes(supplierApiSource, 'prisma.supplier.delete', 'Tenant supplier APIs must not hard delete');

  assertIncludes(requestRoute, 'requireRestaurantStaffAccess(request, restaurantSlug)', 'Tenant purchase request GET uses restaurant staff auth');
  assertIncludes(requestRoute, 'requireRestaurantStaffAccess(request, parsed.data.restaurantSlug, { write: true })', 'Tenant purchase request POST requires OWNER/MANAGER');
  assertIncludes(requestItemRoute, 'requireRestaurantStaffAccess(request, parsed.data.restaurantSlug, { write: true })', 'Tenant purchase request PUT requires OWNER/MANAGER');
  assertIncludes(requestRoute, 'const where = { restaurantId: staff.restaurantId }', 'Tenant purchase request GET scopes by restaurantId');
  assertIncludes(requestRoute, 'where: { id: supplierId, restaurantId, isActive: true }', 'Tenant purchase request validates supplier ownership');
  assertIncludes(requestRoute, 'tx.inventoryItem.findMany', 'Tenant purchase request validates inventory items');
  assertIncludes(requestRoute, 'restaurantId,', 'Tenant purchase request line validation scopes by restaurantId');
  ...
  assertNotIncludes(batch66ApiSource, 'requireAdmin', 'Batch 66 tenant APIs must not use platform requireAdmin');
  assertNotIncludes(batch66ApiSource, 'getAdminFromRequest', 'Batch 66 tenant APIs must not use platform admin session');
  assertNotIncludes(batch66ApiSource, 'prisma.adminUser', 'Batch 66 tenant APIs must not touch AdminUser');
  assertNotIncludes(batch66ApiSource, 'prisma.gatewayLead', 'Batch 66 tenant APIs must not touch GatewayLead');
  assertNotIncludes(batch66ApiSource, 'inventoryItem.update', 'Batch 66 tenant APIs must not mutate inventory stock');
  assertNotIncludes(batch66ApiSource, 'inventoryItem.updateMany', 'Batch 66 tenant APIs must not mutate inventory stock');
  assertNotIncludes(batch66ApiSource, 'inventoryMovement.create', 'Batch 66 tenant APIs must not create inventory movements');
  assertNotIncludes(batch66ApiSource, 'sendWhatsApp', 'Batch 66 tenant APIs must not send WhatsApp');
  assertNotIncludes(batch66ApiSource, 'sendMail', 'Batch 66 tenant APIs must not send email');
  assertNotIncludes(batch66ApiSource, 'nodemailer', 'Batch 66 tenant APIs must not add email transport');
  assertNotIncludes(batch66ApiSource, 'stripe', 'Batch 66 tenant APIs must not add payment logic');
  assertNotIncludes(batch66ApiSource, 'billing', 'Batch 66 tenant APIs must not add billing logic');
  assertNotIncludes(batch66ApiSource, 'domain', 'Batch 66 tenant APIs must not add domain logic');
  assertNotIncludes(batch66ApiSource, 'crm', 'Batch 66 tenant APIs must not add CRM logic');
  assertNotIncludes(batch66ApiSource, 'analytics', 'Batch 66 tenant APIs must not add analytics logic');
  assertNotIncludes(batch66ApiSource, 'autoGenerate', 'Batch 66 tenant APIs must not auto-generate purchase requests');

  assertIncludes(suppliersPage, 'requireRestaurantStaffAccess(cookies(), params.restaurantSlug)', 'Tenant suppliers page validates staff session');
  assertIncludes(requestsPage, 'requireRestaurantStaffAccess(cookies(), params.restaurantSlug)', 'Tenant purchase requests page validates staff session');
  assertIncludes(suppliersClient, 'SUPPORT access is read-only', 'Tenant suppliers SUPPORT read-only state');
  assertIncludes(requestsClient, 'SUPPORT access is read-only', 'Tenant purchase requests SUPPORT read-only state');
  assertIncludes(suppliersClient, 'Refresh', 'Tenant suppliers refresh button');
  assertIncludes(requestsClient, 'Refresh', 'Tenant purchase requests refresh button');
  assertIncludes(suppliersClient, 'No suppliers match the current filters.', 'Tenant suppliers empty state');
  assertIncludes(requestsClient, 'No purchase requests match the current filters.', 'Tenant purchase requests empty state');
  assertIncludes(requestsClient, 'Low-stock helper view', 'Tenant purchase requests low-stock helper view');
  assertIncludes(requestsClient, '/api/restaurant-admin/inventory/items', 'Tenant purchase requests load tenant inventory selector');
  assertIncludes(requestsClient, "'POST'", 'Tenant purchase requests client creates requests via POST');
  assertIncludes(requestsClient, "'PUT'", 'Tenant purchase requests client updates requests via PUT');
  assertIncludes(suppliersClient, "'PUT'", 'Tenant suppliers client updates/deactivates via PUT');
  assertIncludes(suppliersClient, 'No send-to-vendor button', 'Tenant suppliers no vendor sending copy');
  assertIncludes(requestsClient, 'No send-to-vendor action', 'Tenant purchase requests no vendor sending copy');
  assertNotIncludes(batch66UiSource, 'sendToVendor', 'Batch 66 UI must not add send-to-vendor action');
  assertIncludes(tenantNav, "label: 'Suppliers'", 'Tenant admin navigation includes Suppliers');
  assertIncludes(tenantNav, "label: 'Purchase Requests'", 'Tenant admin navigation includes Purchase Requests');
  assertIncludes(tenantAdmin, 'purchaseRequestCounters', 'Tenant admin dashboard includes purchase request counters');
  assertIncludes(tenantAdmin, 'Open suppliers', 'Tenant admin dashboard links to suppliers');
  assertIncludes(tenantAdmin, 'Open purchase requests', 'Tenant admin dashboard links to purchase requests');
  assertIncludes(tenantAdmin, 'prisma.supplier.count', 'Tenant admin dashboard reads supplier counters');
  assertIncludes(tenantAdmin, 'prisma.purchaseRequest.findMany', 'Tenant admin dashboard reads purchase request counters');
  assertIncludes(tenantAdmin, 'restaurantId: staff.restaurantId', 'Tenant admin supplier/request counters scope by restaurantId');

  assertIncludes(readme, 'Tenant supplier and purchase request foundation added.', 'README Batch 66 note');
  assertIncludes(readme, 'Restaurant staff can manage tenant-scoped supplier records and manual purchase requests under `/r/[restaurantSlug]/admin/suppliers` and `/r/[restaurantSlug]/admin/purchase-requests`.', 'README Batch 66 scope note');
  assertIncludes(readme, 'Purchase requests do not mutate inventory stock, create inventory movements, send email/WhatsApp messages, create invoices, process payments, trigger billing/domain/CRM automation, or auto-generate replenishment.', 'README Batch 66 boundary note');
  assertIncludes(blocker, 'Batch 66 adds tenant-scoped supplier records and manual purchase request creation/status tracking.', 'Tenant admin doc Batch 66 note');
  assertIncludes(blocker, 'Manual purchase requests are scoped by restaurantId, validate tenant supplier ownership, and validate tenant inventory item ownership for every line.', 'Tenant admin doc Batch 66 scoping note');
  assertIncludes(blocker, 'Purchase requests track status and line items only; they do not mutate stock, create inventory movements, or send anything to vendors.', 'Tenant admin doc Batch 66 boundary note');

  assert(!fs.existsSync(path.join(root, 'src/app/api/billing')), 'Batch 66 should not add billing API route');
...
function checkTenantPurchaseInvoiceFoundation() {
  const schema = read('prisma/schema.prisma');
  const readme = read('README.md');
  const blocker = read('docs/TENANT_ADMIN_ACCESS_FOUNDATION_BLOCKER.md');
  const migrationPath = path.join(root, 'prisma/migrations/20260612090000_add_purchase_invoices/migration.sql');
  const helperPath = path.join(root, 'src/lib/purchase-invoices.js');
  const invoiceRoutePath = path.join(root, 'src/app/api/restaurant-admin/purchase-invoices/route.js');
  const invoiceItemRoutePath = path.join(root, 'src/app/api/restaurant-admin/purchase-invoices/[id]/route.js');
  const invoicesPagePath = path.join(root, 'src/app/r/[restaurantSlug]/admin/purchase-invoices/page.js');
  ...
  assert(fs.existsSync(migrationPath), 'Batch 68 purchase invoice migration is missing');
  assert(fs.existsSync(helperPath), 'Tenant purchase invoice helper is missing');
  assert(fs.existsSync(invoiceRoutePath), 'Tenant purchase invoice collection API route is missing');
  assert(fs.existsSync(invoiceItemRoutePath), 'Tenant purchase invoice item API route is missing');
  assert(fs.existsSync(invoicesPagePath), 'Tenant purchase invoices admin page is missing');
  assert(fs.existsSync(invoicesClientPath), 'Tenant purchase invoices admin client is missing');

  const migration = read('prisma/migrations/20260612090000_add_purchase_invoices/migration.sql');
  ...
  const invoicesPage = read('src/app/r/[restaurantSlug]/admin/purchase-invoices/page.js');
  ...
  const invoiceUiSource = `${invoicesPage}\n${invoicesClient}\n${tenantNav}\n${tenantAdmin}`;
  const restaurantModel = getModelBlock(schema, 'Restaurant');
  const supplierModel = getModelBlock(schema, 'Supplier');
  const purchaseRequestModel = getModelBlock(schema, 'PurchaseRequest');
  const inventoryItemModel = getModelBlock(schema, 'InventoryItem');
  const purchaseInvoiceModel = getModelBlock(schema, 'PurchaseInvoice');
  const purchaseInvoiceLineModel = getModelBlock(schema, 'PurchaseInvoiceLine');

  assertIncludes(schema, 'model PurchaseInvoice', 'PurchaseInvoice Prisma model');
  assertIncludes(schema, 'model PurchaseInvoiceLine', 'PurchaseInvoiceLine Prisma model');
  assertIncludes(restaurantModel, 'purchaseInvoices   PurchaseInvoice[]', 'Restaurant purchase invoice relation');
  assertIncludes(restaurantModel, 'purchaseInvoiceLines PurchaseInvoiceLine[]', 'Restaurant purchase invoice line relation');
  assertIncludes(supplierModel, 'purchaseInvoices PurchaseInvoice[]', 'Supplier purchase invoice relation');
  assertIncludes(purchaseRequestModel, 'purchaseInvoices PurchaseInvoice[]', 'PurchaseRequest purchase invoice relation');
  assertIncludes(inventoryItemModel, 'purchaseInvoiceLines PurchaseInvoiceLine[]', 'InventoryItem purchase invoice line relation');
  assertIncludes(purchaseInvoiceModel, 'restaurantId        String', 'PurchaseInvoice required restaurantId field');
  assertIncludes(purchaseInvoiceModel, 'restaurant          Restaurant', 'PurchaseInvoice required restaurant relation');
  assertIncludes(purchaseInvoiceModel, 'onDelete: Restrict', 'PurchaseInvoice restaurant FK restricts deletes');
  assertIncludes(purchaseInvoiceLineModel, 'restaurantId      String', 'PurchaseInvoiceLine required restaurantId field');
  assertIncludes(purchaseInvoiceLineModel, 'restaurant        Restaurant', 'PurchaseInvoiceLine required restaurant relation');
  assertIncludes(purchaseInvoiceLineModel, 'onDelete: Restrict', 'PurchaseInvoiceLine restaurant FK restricts deletes');
  assert(!/restaurantId\s+String\?/.test(purchaseInvoiceModel), 'PurchaseInvoice restaurantId must not be nullable');
  assert(!/restaurantId\s+String\?/.test(purchaseInvoiceLineModel), 'PurchaseInvoiceLine restaurantId must not be nullable');
  assertIncludes(purchaseInvoiceModel, 'invoiceNumber       String', 'PurchaseInvoice invoice number field');
  assertIncludes(purchaseInvoiceModel, 'currency            String                @default("AED")', 'PurchaseInvoice currency default');
  assertIncludes(purchaseInvoiceModel, 'subtotal            Float                 @default(0)', 'PurchaseInvoice subtotal field');
  assertIncludes(purchaseInvoiceModel, 'taxAmount           Float                 @default(0)', 'PurchaseInvoice tax amount field');
  assertIncludes(purchaseInvoiceModel, 'totalAmount         Float                 @default(0)', 'PurchaseInvoice total amount field');
  assertIncludes(purchaseInvoiceModel, '@@unique([restaurantId, invoiceNumber])', 'PurchaseInvoice tenant-scoped invoice number unique constraint');
  assertIncludes(purchaseInvoiceModel, '@@index([supplierId])', 'PurchaseInvoice supplier index');
  assertIncludes(purchaseInvoiceModel, '@@index([purchaseRequestId])', 'PurchaseInvoice purchase request index');
  assertIncludes(purchaseInvoiceModel, '@@index([status])', 'PurchaseInvoice status index');
  assertIncludes(purchaseInvoiceModel, '@@index([invoiceDate])', 'PurchaseInvoice invoice date index');

  assertIncludes(migration, 'CREATE TABLE "PurchaseInvoice"', 'PurchaseInvoice migration table');
  assertIncludes(migration, 'CREATE TABLE "PurchaseInvoiceLine"', 'PurchaseInvoiceLine migration table');
  assert((migration.match(/"restaurantId" TEXT NOT NULL/g) || []).length >= 2, 'Batch 68 migration must require invoice restaurantId columns');
  assertNotIncludes(migration, '"restaurantId" TEXT,', 'Batch 68 migration restaurantId columns must not be nullable');
  assertIncludes(migration, 'CREATE UNIQUE INDEX "PurchaseInvoice_restaurantId_invoiceNumber_key" ON "PurchaseInvoice"("restaurantId", "invoiceNumber");', 'PurchaseInvoice migration tenant-scoped invoice number unique index');
  assertIncludes(migration, 'PurchaseInvoice_restaurantId_fkey', 'PurchaseInvoice migration restaurant FK');
  assertIncludes(migration, 'PurchaseInvoiceLine_restaurantId_fkey', 'PurchaseInvoiceLine migration restaurant FK');
  assertIncludes(migration, 'FOREIGN KEY ("restaurantId") REFERENCES "Restaurant"("id") ON DELETE RESTRICT ON UPDATE CASCADE;', 'Batch 68 migration restaurant FKs restrict deletes');

  assertIncludes(helper, 'PURCHASE_INVOICE_STATUSES', 'Purchase invoice helper status constants');
  ...
  assertIncludes(helper, 'normalizePurchaseInvoice', 'Purchase invoice normalizer');
  assertNotIncludes(helper, 'restaurantId:', 'Purchase invoice normalizers must not expose restaurantId');

  assertIncludes(invoiceRoute, 'requireRestaurantStaffAccess(request, restaurantSlug)', 'Tenant purchase invoice GET uses restaurant staff auth');
  assertIncludes(invoiceRoute, 'requireRestaurantStaffAccess(request, parsed.data.restaurantSlug, { write: true })', 'Tenant purchase invoice POST requires OWNER/MANAGER');
  ...
  assertIncludes(invoiceRoute, 'restaurantId,', 'Tenant purchase invoice line validation scopes by restaurantId');
  assertIncludes(invoiceRoute, 'lines: {', 'Tenant purchase invoice creates line items');
  ...
  assertNotIncludes(invoiceApiSource, 'inventoryItem.update', 'Tenant purchase invoice APIs must not mutate inventory stock');
  assertNotIncludes(invoiceApiSource, 'inventoryItem.updateMany', 'Tenant purchase invoice APIs must not mutate inventory stock');
  assertNotIncludes(invoiceApiSource, 'inventoryMovement.create', 'Tenant purchase invoice APIs must not create inventory movements');
  assertNotIncludes(invoiceApiSource, 'sendWhatsApp', 'Tenant purchase invoice APIs must not send WhatsApp');
  assertNotIncludes(invoiceApiSource, 'sendMail', 'Tenant purchase invoice APIs must not send email');
  assertNotIncludes(invoiceApiSource, 'nodemailer', 'Tenant purchase invoice APIs must not add email transport');
  assertNotIncludes(invoiceApiSource, 'stripe', 'Tenant purchase invoice APIs must not add Stripe payment logic');
  assertNotIncludes(invoiceApiSource, 'refund', 'Tenant purchase invoice APIs must not add refund logic');
  assertNotIncludes(invoiceApiSource, 'billing', 'Tenant purchase invoice APIs must not add billing logic');
  assertNotIncludes(invoiceApiSource, 'domain', 'Tenant purchase invoice APIs must not add domain logic');
  assertNotIncludes(invoiceApiSource, 'crm', 'Tenant purchase invoice APIs must not add CRM logic');
  assertNotIncludes(invoiceApiSource, 'payroll', 'Tenant purchase invoice APIs must not add payroll logic');
  assertNotIncludes(invoiceApiSource, 'analytics', 'Tenant purchase invoice APIs must not add analytics logic');
  assertNotIncludes(invoiceApiSource, 'accounting', 'Tenant purchase invoice APIs must not add accounting integration');
  assertNotIncludes(invoiceApiSource, 'taxEngine', 'Tenant purchase invoice APIs must not add a tax engine');
  assertNotIncludes(invoiceApiSource, 'sendToVendor', 'Tenant purchase invoice APIs must not add supplier sending');

  assertIncludes(invoicesPage, 'requireRestaurantStaffAccess(cookies(), params.restaurantSlug)', 'Tenant purchase invoices page validates staff session');
  assertIncludes(invoicesClient, 'SUPPORT access is read-only', 'Tenant purchase invoices SUPPORT read-only state');
  assertIncludes(invoicesClient, 'Refresh', 'Tenant purchase invoices refresh button');
  assertIncludes(invoicesClient, 'No purchase invoices match the current filters.', 'Tenant purchase invoices empty state');
  assertIncludes(invoicesClient, '/api/restaurant-admin/purchase-invoices', 'Tenant purchase invoices client uses purchase invoices API');
  assertIncludes(invoicesClient, '/api/restaurant-admin/suppliers', 'Tenant purchase invoices client loads supplier selector');
  assertIncludes(invoicesClient, '/api/restaurant-admin/purchase-requests', 'Tenant purchase invoices client loads purchase request selector');
  ...
  assertIncludes(tenantNav, "label: 'Purchase Invoices'", 'Tenant admin navigation includes Purchase Invoices');
  assertIncludes(tenantAdmin, 'purchaseInvoiceCounters', 'Tenant admin dashboard includes purchase invoice counters');
  assertIncludes(tenantAdmin, 'Open purchase invoices', 'Tenant admin dashboard links to purchase invoices');
  assertIncludes(tenantAdmin, 'prisma.purchaseInvoice.findMany', 'Tenant admin dashboard reads purchase invoice counters');
  assertIncludes(tenantAdmin, 'restaurantId: staff.restaurantId', 'Tenant admin purchase invoice counters scope by restaurantId');

  assertIncludes(readme, 'Tenant purchase invoice recording foundation added.', 'README Batch 68 note');
  assertIncludes(readme, 'Restaurant staff can record tenant-scoped supplier purchase invoices under `/r/[restaurantSlug]/admin/purchase-invoices`.', 'README Batch 68 scope note');
  assertIncludes(readme, 'Purchase invoice lines validate tenant inventory item ownership when linked, and supplier/request links validate tenant ownership.', 'README Batch 68 validation note');
  assertIncludes(readme, 'Purchase invoices do not mutate inventory stock, create inventory movements, create payments/refunds, send supplier/email/WhatsApp messages, or trigger billing/domain/CRM/payroll/accounting/tax/analytics automation.', 'README Batch 68 boundary note');
  assertIncludes(blocker, 'tenant purchase invoice recording foundation resolved by Batch 68', 'Tenant admin doc Batch 68 status');
  assertIncludes(blocker, 'Batch 68 adds tenant-scoped purchase invoice recording with invoice lines, optional supplier links, optional purchase request links, and status tracking.', 'Tenant admin doc Batch 68 behavior');
  assertIncludes(blocker, 'Purchase invoice recording validates tenant ownership for supplier, purchase request, and linked inventory item references.', 'Tenant admin doc Batch 68 validation boundary');
  assertIncludes(blocker, 'Purchase invoices do not mutate stock, create inventory movements, create payments/refunds, or send anything to vendors.', 'Tenant admin doc Batch 68 automation boundary');

  assert(!fs.existsSync(path.join(root, 'src/app/api/billing')), 'Batch 68 should not add billing API route');
...
function checkTenantPurchaseInvoicePaymentRecordingFoundation() {
  ...
  assertIncludes(invoiceItemRoute, 'await prisma.$transaction(', 'Tenant purchase invoice PUT uses a transaction');
...
function checkTenantOperationsReportingFoundation() {
  ...
  assert(/prisma\.purchaseInvoice\.groupBy\(\{\s*by: \['currency'\]/.test(helper), 'Tenant reports helper groups purchase invoice totals by currency');
  ...
  assert(/prisma\.purchaseInvoicePayment\.groupBy\(\{\s*by: \['currency'\]/.test(helper), 'Tenant reports helper groups recorded payment amounts by currency');
...
function checkTenantAuditLoggingSecurityHardening() {
  const schema = read('prisma/schema.prisma');
  const readme = read('README.md');
  const blocker = read('docs/TENANT_ADMIN_ACCESS_FOUNDATION_BLOCKER.md');
  const packageJson = read('package.json');
  const migrationPath = path.join(root, 'prisma/migrations/20260613160000_add_restaurant_audit_logs/migration.sql');
  const helperPath = path.join(root, 'src/lib/tenant-audit.js');
  const auditRoutePath = path.join(root, 'src/app/api/restaurant-admin/audit-logs/route.js');
  const auditItemRoutePath = path.join(root, 'src/app/api/restaurant-admin/audit-logs/[id]/route.js');
  const auditPagePath = path.join(root, 'src/app/r/[restaurantSlug]/admin/audit-logs/page.js');
  const auditClientPath = path.join(root, 'src/app/r/[restaurantSlug]/admin/audit-logs/TenantAuditLogsClient.jsx');

  assert(fs.existsSync(migrationPath), 'Batch 71 audit log migration is missing');
  assert(fs.existsSync(helperPath), 'Tenant audit helper is missing');
  assert(fs.existsSync(auditRoutePath), 'Tenant audit log API route is missing');
  assert(!fs.existsSync(auditItemRoutePath), 'Tenant audit log item route must not exist');
  assert(fs.existsSync(auditPagePath), 'Tenant audit log admin page is missing');
  assert(fs.existsSync(auditClientPath), 'Tenant audit log admin client is missing');

  const migration = read('prisma/migrations/20260613160000_add_restaurant_audit_logs/migration.sql');
  const helper = read('src/lib/tenant-audit.js');
  ...
  const middleware = read('middleware.js');
  const restaurantModel = getModelBlock(schema, 'Restaurant');
  const auditModel = getModelBlock(schema, 'RestaurantAuditLog');

  assertIncludes(restaurantModel, 'auditLogs         RestaurantAuditLog[]', 'Restaurant audit log relation');
  assertIncludes(auditModel, 'restaurantId            String', 'RestaurantAuditLog required restaurantId field');
  assertIncludes(auditModel, 'restaurant              Restaurant', 'RestaurantAuditLog required restaurant relation');
  assertIncludes(auditModel, 'onDelete: Restrict', 'RestaurantAuditLog restaurant FK restricts deletes');
  assertIncludes(auditModel, 'actorRestaurantUserId   String?', 'RestaurantAuditLog actor id field');
  assertIncludes(auditModel, 'action                  String', 'RestaurantAuditLog action field');
  assertIncludes(auditModel, 'entityType              String', 'RestaurantAuditLog entity type field');
  assertIncludes(auditModel, 'metadata                Json?', 'RestaurantAuditLog metadata field');
  assertIncludes(auditModel, '@@index([restaurantId])', 'RestaurantAuditLog restaurant index');
  assertIncludes(auditModel, '@@index([actorRestaurantUserId])', 'RestaurantAuditLog actor index');
  assertIncludes(auditModel, '@@index([action])', 'RestaurantAuditLog action index');
  assertIncludes(auditModel, '@@index([entityType])', 'RestaurantAuditLog entity type index');
  assertIncludes(auditModel, '@@index([entityId])', 'RestaurantAuditLog entity id index');
  assertIncludes(auditModel, '@@index([createdAt])', 'RestaurantAuditLog createdAt index');
  assertNotIncludes(auditModel, 'restaurantId            String?', 'RestaurantAuditLog restaurantId must not be nullable');
  assertIncludes(migration, 'CREATE TABLE "RestaurantAuditLog"', 'RestaurantAuditLog migration table');
  assertIncludes(migration, '"restaurantId" TEXT NOT NULL', 'RestaurantAuditLog migration requires restaurantId');
  ...
  assertIncludes(helper, 'SENSITIVE_METADATA_KEYS', 'Tenant audit helper sensitive metadata key list');
  for (const secretToken of ['password', 'passwordHash', 'cookie', 'session', 'DATABASE_URL', 'secret']) {
    assertIncludes(helper, secretToken, `Tenant audit helper sanitizes ${secretToken}`);
  }
  assertIncludes(helper, 'console.error', 'Tenant audit helper logs audit write failures server-side');
  assertIncludes(helper, 'normalizeTenantAuditLog', 'Tenant audit normalizer exists');
  assertNotIncludes(helper.match(/export function normalizeTenantAuditLog[\s\S]*?\n\}/)?.[0] || '', 'restaurantId', 'Tenant audit normalizer must not expose restaurantId');

  assertIncludes(auditRoute, 'export async function GET', 'Tenant audit API exposes GET');
  assertNotIncludes(auditRoute, 'export async function POST', 'Tenant audit API must not expose POST');
  assertNotIncludes(auditRoute, 'export async function PUT', 'Tenant audit API must not expose PUT');
  assertNotIncludes(auditRoute, 'export async function PATCH', 'Tenant audit API must not expose PATCH');
  assertNotIncludes(auditRoute, 'export async function DELETE', 'Tenant audit API must not expose DELETE');
  ...
  assertNotIncludes(auditRoute, 'prisma.adminUser', 'Tenant audit API must not touch AdminUser');
  assertNotIncludes(auditRoute, 'prisma.gatewayLead', 'Tenant audit API must not touch GatewayLead');

  assertIncludes(auditPage, 'requireRestaurantStaffAccess(cookies(), params.restaurantSlug)', 'Tenant audit page validates staff session');
  assertIncludes(auditPage, 'isRestaurantStaffWriteRole(staff.role)', 'Tenant audit page OWNER/MANAGER role gate');
  assertIncludes(auditClient, 'Audit logs are read-only.', 'Tenant audit UI read-only copy');
  assertIncludes(auditClient, 'Logs are tenant-scoped.', 'Tenant audit UI tenant-scoped copy');
  assertIncludes(auditClient, 'No external logging or alerting is connected.', 'Tenant audit UI no external logging copy');
  assertIncludes(auditClient, 'Refresh', 'Tenant audit UI refresh button');
  assertIncludes(auditClient, 'metadataSummary', 'Tenant audit UI shows sanitized metadata summary only');
  assertNotIncludes(auditClient, "method: 'POST'", 'Tenant audit UI must not call POST');
  assertNotIncludes(auditClient, "method: 'PUT'", 'Tenant audit UI must not call PUT');
  assertNotIncludes(auditClient, "method: 'DELETE'", 'Tenant audit UI must not call DELETE');
  assertIncludes(tenantNav, "staff && isRestaurantStaffWriteRole(staff.role)", 'Tenant audit nav hides audit logs from SUPPORT');
  ...
  const settingsRoute = read('src/app/api/restaurant-admin/settings/route.js');
  const profileRoute = read('src/app/api/restaurant-admin/profile/route.js');
  const inventoryItemsRoute = read('src/app/api/restaurant-admin/inventory/items/route.js');
  const inventoryItemRoute = read('src/app/api/restaurant-admin/inventory/items/[id]/route.js');
  const inventoryMovementsRoute = read('src/app/api/restaurant-admin/inventory/movements/route.js');
  const recipeApplyRoute = read('src/app/api/restaurant-admin/recipes/apply/route.js');
  const purchaseRequestRoute = read('src/app/api/restaurant-admin/purchase-requests/route.js');
  const purchaseRequestItemRoute = read('src/app/api/restaurant-admin/purchase-requests/[id]/route.js');
  const purchaseReceiveRoute = read('src/app/api/restaurant-admin/purchase-requests/[id]/receive/route.js');
  const purchaseInvoiceRoute = read('src/app/api/restaurant-admin/purchase-invoices/route.js');
  const purchaseInvoiceItemRoute = read('src/app/api/restaurant-admin/purchase-invoices/[id]/route.js');
  const paymentRoute = read('src/app/api/restaurant-admin/purchase-invoices/[id]/payments/route.js');
  const paymentItemRoute = read('src/app/api/restaurant-admin/purchase-invoices/[id]/payments/[paymentId]/route.js');
  const orderItemRoute = read('src/app/api/restaurant-admin/orders/[id]/route.js');
  const reservationItemRoute = read('src/app/api/restaurant-admin/reservations/[id]/route.js');
  const auditedWriteSources = [
    staffRoute,
    staffItemRoute,
    settingsRoute,
    profileRoute,
    ...
    recipeApplyRoute,
    purchaseRequestRoute,
    purchaseRequestItemRoute,
    purchaseReceiveRoute,
  ...
  assertNotIncludes(auditedWriteSources, 'prisma.adminUser', 'Tenant audited APIs must not touch AdminUser');
  assertNotIncludes(auditedWriteSources, 'prisma.gatewayLead', 'Tenant audited APIs must not touch GatewayLead');

  assertNotIncludes(middleware, 'prisma', 'Tenant admin middleware must not import Prisma');
  assertIncludes(auditRoute, 'staff.restaurantId', 'Tenant audit API must use DB-backed staff restaurant scope');
  for (const source of [staffRoute, settingsRoute, inventoryItemsRoute, purchaseRequestRoute, purchaseInvoiceRoute, paymentRoute]) {
    assertIncludes(source, 'requireRestaurantStaffAccess', 'Tenant operational APIs use DB-backed restaurant staff access');
    assertNotIncludes(source, 'requireAdmin', 'Tenant operational APIs must not use platform requireAdmin');
    assertNotIncludes(source, 'AdminUser', 'Tenant operational APIs must not use AdminUser');
  }
  const platformApiSource = [
    read('src/app/api/admin/users/route.js'),
    read('src/app/api/admin/users/[id]/route.js'),
    read('src/app/api/admin/gateway-leads/route.js'),
    read('src/app/api/admin/gateway-leads/[id]/route.js'),
  ].join('\n');
  assertNotIncludes(platformApiSource, 'requireRestaurantStaffAccess', 'Platform admin APIs must not use restaurant staff auth');
  const publicTenantWriteSource = [
    read('src/app/api/orders/route.js'),
    read('src/app/api/reservations/route.js'),
    read('src/app/api/orders/tenant-cancel/route.js'),
    read('src/app/api/reservations/tenant-cancel/route.js'),
  ].join('\n');
  assertIncludes(publicTenantWriteSource, 'restaurantSlug', 'Public tenant write APIs remain slug scoped');
  assertIncludes(publicTenantWriteSource, 'restaurantId', 'Public tenant write APIs remain restaurant scoped');
  const databaseUrlFiles = [];
  const scanDatabaseUrl = (relativePath) => {
    const fullPath = path.join(root, relativePath);
    for (const entry of fs.readdirSync(fullPath, { withFileTypes: true })) {
      const childRelativePath = path.join(relativePath, entry.name);
      if (entry.isDirectory()) {
        if (!['node_modules', '.next', '.git'].includes(entry.name)) scanDatabaseUrl(childRelativePath);
      } else if (/\.(js|jsx|prisma)$/.test(entry.name) && read(childRelativePath).includes('DATABASE_URL')) {
        databaseUrlFiles.push(childRelativePath.replace(/\\/g, '/'));
      }
    }
  };
  scanDatabaseUrl('src');
  scanDatabaseUrl('prisma');
  const expectedDatabaseUrlFiles = new Set([
    'src/app/action.js',
    'src/app/api/gateway/leads/route.js',
    'src/app/platform-admin/(protected)/client-restaurants/page.js',
    'src/app/platform-admin/(protected)/page.js',
    'src/app/public/order/page.js',
    'src/app/public/table/[slug]/page.js',
    'src/app/r/[restaurantSlug]/table/[slug]/page.js',
    'src/lib/restaurants.js',
    'src/lib/restaurant-profile.js',
    'src/lib/tenant-audit.js',
    'prisma/schema.prisma',
    'prisma/seed.js',
  ]);
  const unexpectedDatabaseUrlFiles = databaseUrlFiles.filter((file) => !expectedDatabaseUrlFiles.has(file));
  assert(!unexpectedDatabaseUrlFiles.length, `Unexpected DATABASE_URL references: ${unexpectedDatabaseUrlFiles.join(', ')}`);
  for (const forbidden of ['winston', 'pino', 'logtail', 'sentry', 'datadog', 'stripe', 'refund', 'sendWhatsApp', 'sendMail', 'nodemailer', 'billing', 'domain', 'crm', 'payroll', 'taxEngine', 'accounting', 'analyticsEngine', 'siem']) {
    assertNotIncludes(`${helper}\n${auditRoute}\n${auditClient}\n${auditedWriteSources}`, forbidden, `Batch 71 must not add ${forbidden} behavior`);
    assertNotIncludes(packageJson, forbidden, `Batch 71 must not add ${forbidden} package dependency`);
  }

  assertIncludes(readme, 'Tenant audit logging and security hardening foundation added.', 'README Batch 71 audit note');
  assertIncludes(readme, 'Audit logs are tenant-scoped, read-only for OWNER and MANAGER staff, and sanitize sensitive metadata.', 'README Batch 71 scope note');
  assertIncludes(blocker, 'tenant audit logging and security hardening foundation resolved by Batch 71', 'Tenant admin doc Batch 71 status');
...
function checkTenantPaymentSettingsFoundation() {
  const schema = read('prisma/schema.prisma');
  const readme = read('README.md');
  const blocker = read('docs/TENANT_ADMIN_ACCESS_FOUNDATION_BLOCKER.md');
  const boundary = read('docs/PAYMENT_INTEGRATION_BOUNDARY.md');
  const providerReadiness = read('docs/PAYMENT_PROVIDER_READINESS_CHECKLIST.md');
  const productionReadiness = read('docs/PRODUCTION_READINESS_CHECKLIST.md');
  const security = read('docs/SECURITY_HARDENING_CHECKLIST.md');
  const packageJson = read('package.json');
  const migrationPath = path.join(root, 'prisma/migrations/20260614170000_add_restaurant_payment_settings/migration.sql');
  const helperPath = path.join(root, 'src/lib/payment-settings.js');
  const apiPath = path.join(root, 'src/app/api/restaurant-admin/payment-settings/route.js');
  const pagePath = path.join(root, 'src/app/r/[restaurantSlug]/admin/payment-settings/page.js');
  const clientPath = path.join(root, 'src/app/r/[restaurantSlug]/admin/payment-settings/TenantPaymentSettingsClient.jsx');

  assert(fs.existsSync(migrationPath), 'Batch 74 focused payment settings migration is missing');
  assert(fs.existsSync(helperPath), 'Batch 74 payment settings helper is missing');
  assert(fs.existsSync(apiPath), 'Batch 74 payment settings API route is missing');
  assert(fs.existsSync(pagePath), 'Batch 74 tenant payment settings page is missing');
  assert(fs.existsSync(clientPath), 'Batch 74 tenant payment settings client is missing');

  const migration = read('prisma/migrations/20260614170000_add_restaurant_payment_settings/migration.sql');
  const helper = read('src/lib/payment-settings.js');
  ...
  assertIncludes(schema, 'paymentSettings', 'RestaurantSettings paymentSettings schema field');
  assertIncludes(migration, 'ALTER TABLE "RestaurantSettings" ADD COLUMN "paymentSettings" TEXT', 'Batch 74 paymentSettings migration column');
  assertIncludes(readme, 'focused nullable `paymentSettings` field', 'README Batch 74 migration justification');
  ...
  for (const token of [
    ...
    assertIncludes(helper, token, `Batch 74 payment settings helper ${token}`);
  }
  for (const token of ['DISABLED', 'TEST_PLANNED', 'LIVE_PLANNED', 'NONE', 'PROVIDER_PLACEHOLDER']) {
    assertIncludes(helper, token, `Batch 74 payment helper constant ${token}`);
  }
  ...
  assertNotIncludes(helper, 'stripe', 'Batch 74 payment settings helper must not import Stripe');

  assertIncludes(apiRoute, 'requireRestaurantStaffAccess(request, restaurantSlug)', 'Payment settings GET uses staff auth');
  assertIncludes(apiRoute, 'requireRestaurantStaffAccess(request, parsed.data.restaurantSlug, { write: true })', 'Payment settings PUT requires OWNER/MANAGER');
  ...
  assertIncludes(apiRoute, 'paymentSettings: serializePaymentSettings(nextPaymentSettings)', 'Payment settings API persists only normalized payment settings');
  assertIncludes(apiRoute, 'createTenantAuditLog', 'Payment settings API writes tenant audit log');
  assertIncludes(apiRoute, 'TENANT_AUDIT_ACTIONS.PAYMENT_SETTINGS_UPDATED', 'Payment settings API uses payment settings audit action');
  assertIncludes(auditHelper, 'PAYMENT_SETTINGS_UPDATED', 'Tenant audit helper includes payment settings action');
  assertNotIncludes(apiRoute, 'parsed.data.restaurantId', 'Payment settings API must not persist restaurantId from body');
  assertNotIncludes(apiRoute, 'prisma.order', 'Payment settings API must not mutate orders');
  assertNotIncludes(apiRoute, 'prisma.purchaseInvoice', 'Payment settings API must not mutate invoices');
  ...
  assertIncludes(client, 'No real payment processing is enabled.', 'Payment settings UI no real processing copy');
  ...
  for (const readinessText of [
    ...
    'webhook endpoint planned externally',
    'secrets stored in hosting provider, not database',
  ]) {
    assertIncludes(client, readinessText, `Payment settings UI readiness checklist ${readinessText}`);
  }
  for (const forbiddenInput of ['secret key', 'webhook signing secret', 'PAYMENT_SECRET_KEY', 'PAYMENT_WEBHOOK_SIGNING_SECRET']) {
    assertNotIncludes(client, forbiddenInput, `Payment settings UI must not expose ${forbiddenInput} input`);
  }
  assertNotIncludes(client, 'provider endpoint', 'Payment settings UI must not call provider endpoints');
  assertIncludes(tenantNav, "label: 'Payment Settings'", 'Tenant nav includes Payment Settings');
  assertIncludes(tenantAdmin, 'paymentSettingsSummary', 'Tenant dashboard includes payment settings summary');
  assertIncludes(tenantAdmin, 'Open payment settings', 'Tenant dashboard links to payment settings');
  assertIncludes(tenantAdmin, 'No provider secrets are stored in the database.', 'Tenant dashboard payment settings boundary copy');

  for (const doc of [readme, blocker, boundary, providerReadiness, productionReadiness, security]) {
    assertIncludes(doc, 'tenant payment settings foundation', 'Batch 74 docs mention tenant payment settings foundation');
    assertIncludes(doc, 'no real payment processing', 'Batch 74 docs preserve no real payment processing boundary');
    assertIncludes(doc, 'no provider calls', 'Batch 74 docs preserve no provider calls boundary');
    assertIncludes(doc, 'no secrets stored in database', 'Batch 74 docs preserve no database secrets boundary');
  }
  assertIncludes(readme, 'Tenant payment settings foundation added.', 'README Batch 74 note');
  assertIncludes(blocker, 'tenant payment settings foundation resolved by Batch 74', 'Tenant admin blocker Batch 74 status');
  ...
  assert(!fs.existsSync(path.join(root, 'src/app/api/webhooks')), 'Batch 74 must not add webhook API route');

  assertNotIncludes(packageJson, 'stripe', 'Batch 74 should not add Stripe dependency');
  assertNotIncludes(packageJson, '@stripe', 'Batch 74 should not add Stripe scoped dependency');
  assertNotIncludes(packageJson, 'paypal', 'Batch 74 should not add PayPal dependency');
  assertNotIncludes(packageJson, 'checkout', 'Batch 74 should not add checkout dependency');
}

function checkPaymentWebhookIdempotencyFoundation() {
  const schema = read('prisma/schema.prisma');
  const readme = read('README.md');
  const blocker = read('docs/TENANT_ADMIN_ACCESS_FOUNDATION_BLOCKER.md');
  const boundary = read('docs/PAYMENT_INTEGRATION_BOUNDARY.md');
  ...
  const security = read('docs/SECURITY_HARDENING_CHECKLIST.md');
  const packageJson = read('package.json');
  const migrationPath = path.join(root, 'prisma/migrations/20260615090000_add_payment_provider_events/migration.sql');
  const helperPath = path.join(root, 'src/lib/payment-provider-events.js');
  const apiPath = path.join(root, 'src/app/api/restaurant-admin/payment-provider-events/route.js');
  const pagePath = path.join(root, 'src/app/r/[restaurantSlug]/admin/payment-provider-events/page.js');
  const clientPath = path.join(root, 'src/app/r/[restaurantSlug]/admin/payment-provider-events/TenantPaymentProviderEventsClient.jsx');

  assert(fs.existsSync(migrationPath), 'Batch 75 payment provider event migration is missing');
  assert(fs.existsSync(helperPath), 'Batch 75 payment provider event helper is missing');
  assert(fs.existsSync(apiPath), 'Batch 75 payment provider event API route is missing');
  assert(fs.existsSync(pagePath), 'Batch 75 tenant payment provider events page is missing');
  assert(fs.existsSync(clientPath), 'Batch 75 tenant payment provider events client is missing');

  const migration = read('prisma/migrations/20260615090000_add_payment_provider_events/migration.sql');
  const helper = read('src/lib/payment-provider-events.js');
  const apiRoute = read('src/app/api/restaurant-admin/payment-provider-events/route.js');
  const page = read('src/app/r/[restaurantSlug]/admin/payment-provider-events/page.js');
  const client = read('src/app/r/[restaurantSlug]/admin/payment-provider-events/TenantPaymentProviderEventsClient.jsx');
  const tenantNav = read('src/app/r/[restaurantSlug]/admin/TenantAdminNav.jsx');
  const tenantAdmin = read('src/app/r/[restaurantSlug]/admin/page.js');
  const docsSource = `${readme}\n${blocker}\n${boundary}\n${providerReadiness}\n${productionReadiness}\n${security}`;
  const batch75Source = `${helper}\n${apiRoute}\n${page}\n${client}\n${tenantNav}\n${tenantAdmin}`;

  assertIncludes(schema, 'model PaymentProviderEvent', 'PaymentProviderEvent Prisma model');
  assertIncludes(schema, '@@unique([provider, providerMode, providerEventId])', 'PaymentProviderEvent provider/mode/event unique constraint');
  assertIncludes(schema, '@@index([relatedEntityType, relatedEntityId])', 'PaymentProviderEvent related entity composite index');
  assert(/metadata\s+Json\?/.test(schema), 'PaymentProviderEvent sanitized metadata field');
  assert(/restaurantId\s+String\?/.test(schema), 'PaymentProviderEvent nullable restaurantId for unresolved future events');
  assertIncludes(migration, 'CREATE TABLE "PaymentProviderEvent"', 'PaymentProviderEvent migration table');
  assertIncludes(migration, '"restaurantId" TEXT', 'PaymentProviderEvent migration nullable restaurantId column');
  assertIncludes(migration, '"metadata" JSONB', 'PaymentProviderEvent migration JSONB metadata');
  assertIncludes(migration, 'PaymentProviderEvent_provider_providerMode_providerEventId_key', 'PaymentProviderEvent migration unique event key');
  assertIncludes(migration, 'PaymentProviderEvent_relatedEntityType_relatedEntityId_idx', 'PaymentProviderEvent migration related entity index');
  ...
  for (const token of [
    ...
    'TEST',
    'LIVE',
    'sanitizePaymentProviderEventMetadata',
  ...
  for (const unsafeKey of ['authorization', 'cookie', 'signature', 'card', 'cvc', 'payload', 'secret', 'token', 'header']) {
  ...
  assertNotIncludes(helper, 'paypal', 'Batch 75 helper must not import PayPal');

  assertIncludes(apiRoute, 'requireRestaurantStaffAccess(request, restaurantSlug)', 'Payment provider events API uses staff auth');
  assertIncludes(apiRoute, 'isRestaurantStaffWriteRole(staff.role)', 'Payment provider events API is OWNER/MANAGER only');
  assertIncludes(apiRoute, 'const where = { restaurantId: staff.restaurantId }', 'Payment provider events API scopes reads by staff restaurantId');
  assertIncludes(apiRoute, 'MAX_PAYMENT_PROVIDER_EVENT_LIMIT = 100', 'Payment provider events API caps limit at 100');
  assertIncludes(apiRoute, 'MAX_PAYMENT_PROVIDER_EVENT_RANGE_DAYS = 366', 'Payment provider events API caps date range at 366 days');
  assertIncludes(apiRoute, 'From and To dates are required when filtering payment provider events', 'Payment provider events API rejects one-sided date filters');
  assertIncludes(apiRoute, 'providerMode', 'Payment provider events API supports providerMode filter');
  assertIncludes(apiRoute, 'eventType', 'Payment provider events API supports eventType filter');
  assertIncludes(apiRoute, 'normalizePaymentProviderEvents', 'Payment provider events API uses safe normalizer');
  for (const method of ['POST', 'PUT', 'PATCH', 'DELETE']) {
    assertNotIncludes(apiRoute, `export async function ${method}`, `Payment provider events API must not implement ${method}`);
  }
  assertNotIncludes(apiRoute, 'create({', 'Payment provider events API must not create events');
  assertNotIncludes(apiRoute, 'update({', 'Payment provider events API must not update events');
  assertNotIncludes(apiRoute, 'updateMany', 'Payment provider events API must not update events');
  assertNotIncludes(apiRoute, 'delete', 'Payment provider events API must not delete events');

  assertIncludes(page, 'requireRestaurantStaffAccess(cookies(), params.restaurantSlug)', 'Payment provider events page uses DB-backed staff guard');
  assertIncludes(page, 'isRestaurantStaffWriteRole(staff.role)', 'Payment provider events page is OWNER/MANAGER only');
  assertIncludes(client, 'Provider event tracking is a foundation only.', 'Payment provider events UI foundation copy');
  ...
  for (const forbiddenControl of ['Save', 'Create', 'Update', 'Delete', 'Void', 'Refund', 'Checkout']) {
    assertNotIncludes(client, `>${forbiddenControl}<`, `Payment provider events UI must not expose ${forbiddenControl} mutation control`);
  }
  assertIncludes(tenantNav, "label: 'Payment Events'", 'Tenant nav includes Payment Events');
  assertIncludes(tenantNav, "key: 'payment-provider-events'", 'Tenant nav payment events key');
  assertIncludes(tenantAdmin, 'paymentProviderEventCount', 'Tenant dashboard includes payment provider event count');
  assertIncludes(tenantAdmin, 'Open payment events', 'Tenant dashboard links to payment events');
  assertIncludes(tenantAdmin, 'No webhook endpoint is active.', 'Tenant dashboard payment event boundary copy');

  for (const doc of [readme, blocker, boundary, providerReadiness, productionReadiness, security]) {
    assertIncludes(doc, 'payment webhook idempotency foundation', 'Batch 75 docs mention payment webhook idempotency foundation');
    assertIncludes(doc, 'no webhook endpoint is active', 'Batch 75 docs preserve no active webhook endpoint boundary');
    assertIncludes(doc, 'no real payment processing', 'Batch 75 docs preserve no real payment processing boundary');
  ...
  collectApiRoutes('src/app/api');
  assert(!apiRouteFiles.some((file) => /checkout|session|refund|webhook/i.test(file)), 'Batch 75 must not add checkout/session/refund/webhook API routes');
  assert(!fs.existsSync(path.join(root, 'src/app/api/payment-provider-events')), 'Batch 75 must not add public payment provider events API route');
  assert(!fs.existsSync(path.join(root, 'src/app/api/payments')), 'Batch 75 must not add generic payments API route');
  assert(!fs.existsSync(path.join(root, 'src/app/api/refunds')), 'Batch 75 must not add refund API route');
  assert(!fs.existsSync(path.join(root, 'src/app/api/webhooks')), 'Batch 75 must not add webhook API route');
  ...
  for (const forbidden of [
    ...
    assertNotIncludes(batch75Source, forbidden, `Batch 75 must not add provider/payment implementation ${forbidden}`);
  }
  for (const forbidden of [
    'PAYMENT_SECRET_KEY=',
    'PAYMENT_WEBHOOK_SIGNING_SECRET=',
    ...
    assertNotIncludes(docsSource, forbidden, `Batch 75 docs must not include secret example ${forbidden}`);
  }

  assertNotIncludes(packageJson, 'stripe', 'Batch 75 should not add Stripe dependency');
  assertNotIncludes(packageJson, '@stripe', 'Batch 75 should not add Stripe scoped dependency');
  assertNotIncludes(packageJson, 'paypal', 'Batch 75 should not add PayPal dependency');
  assertNotIncludes(packageJson, 'checkout', 'Batch 75 should not add checkout dependency');
}
...
function checkInventoryFoundation() {
  ...
  assertIncludes(readme, 'No automatic stock deduction or automatic supplier request generation has been added yet.', 'README inventory automation limitation');
}
...
function checkInventoryLowStockUxFilters() {
  ...
  assertIncludes(readme, 'Still no automatic deduction or automatic supplier sending has been added.', 'README inventory polish automation limitation');
}
...
function checkRecipeIngredientMappingFoundation() {
  ...
  assertIncludes(readme, 'no automatic supplier sending', 'README recipe supplier limitation');
...
function checkRecipeMappingUxPolish() {
  ...
  assertIncludes(readme, 'no automatic supplier sending', 'README recipe UX supplier limitation');
...
function checkRecipeConsumptionDryRun() {
  ...
  assertIncludes(readme, 'no automatic supplier sending', 'README recipe dry-run supplier limitation');
}
...
function checkManualRecipeConsumptionApply() {
  ...
  assertIncludes(readme, 'no automatic supplier sending', 'README manual recipe supplier limitation');
}
...
const checks = [
  ...
  checkTenantPaymentSettingsFoundation,
  checkPaymentWebhookIdempotencyFoundation,
