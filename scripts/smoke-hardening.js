...
function checkTenantProfileSettingsAdmin() {
  ...
  assertIncludes(readme, 'Feature/module activation, ordering activation, orders, reservations, inventory, recipes, broader staff management, billing, domains, email, and WhatsApp automation remain future work.', 'README Batch 52 future boundary note');
...
function checkTenantStaffManagementFoundation() {
  ...
  assertIncludes(itemRoute, 'prisma.restaurantUser.update', 'Tenant staff PUT updates RestaurantUser');
  ...
  assertIncludes(itemRoute, 'ensureCanChangeOwnerStatus', 'Tenant staff PUT has last-owner protection helper');
  assertIncludes(itemRoute, "targetStaffUser.role === 'OWNER'", 'Tenant staff PUT checks target OWNER role');
  assertIncludes(itemRoute, 'targetStaffUser.isActive', 'Tenant staff PUT checks target active state');
  assertIncludes(itemRoute, 'activeOwnerCount <= 1', 'Tenant staff PUT protects last active OWNER');
