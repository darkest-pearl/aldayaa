...
- The payment webhook idempotency foundation stores provider-neutral event/idempotency records for future provider webhook handling and tenant read-only visibility.
- Payment provider event records are scoped by restaurantId when tenant ownership is known and remain nullable only for unresolved future events.
