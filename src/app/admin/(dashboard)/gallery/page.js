...
export default function AdminGalleryPage() {
  ...
  const addCategory = async (e) => {
    e.preventDefault();
    await fetchJson('/api/gallery/categories', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(catForm) });
    setCatForm({ name: '' });
    load();
  };
  ...
  const addPhoto = async (e) => {
    e.preventDefault();
    await fetchJson('/api/gallery/photos', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(photoForm) });
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="section-bg p-4">
          ...
          <form className="grid gap-2" onSubmit={addPhoto}>
            <input className="border rounded p-2" placeholder="Title" value={photoForm.title} onChange={(e) => setPhotoForm({ ...photoForm, title: e.target.value })} />
            <textarea className="border rounded p-2" placeholder="Description" value={photoForm.description} onChange={(e) => setPhotoForm({ ...photoForm, description: e.target.value })} />
            <select className="border rounded p-2" value={photoForm.categoryId} onChange={(e) => setPhotoForm({ ...photoForm, categoryId: e.target.value })}>
              <option value="">Select category</option>
              {categories.map((cat) => <option key={cat.id} value={cat.id}>{cat.name}</option>)}
            </select>
            <input className="border rounded p-2" placeholder="Image URL" value={photoForm.imageUrl} onChange={(e) => setPhotoForm({ ...photoForm, imageUrl: e.target.value })} />
            ...
            <div className="grid sm:grid-cols-2 gap-3 mt-3 text-sm">
              {cat.photos?.map((photo) => (
                <div key={photo.id} className="border rounded p-2 flex justify-between">
                  ...
                  <button className="text-red-600" onClick={() => deletePhoto(photo.id)}>Delete</button>
                </div>
              ))}
            </div>
          </div>
