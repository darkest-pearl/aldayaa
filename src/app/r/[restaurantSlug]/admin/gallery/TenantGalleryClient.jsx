...
export default function TenantGalleryClient({ restaurantSlug, staffRole }) {
  ...
  async function updateCategory(event) {
    ...
    try {
      await apiRequest(`/api/restaurant-admin/gallery/categories/${editingCategoryId}`, {
        method: 'PUT',
        body: JSON.stringify({ restaurantSlug, name: categoryForm.name }),
      });
      setCategoryForm({ name: '' });
  ...
  async function updatePhoto(event) {
    event.preventDefault();
    if (!writable || !editingPhotoId) return;
    try {
      await apiRequest(`/api/restaurant-admin/gallery/photos/${editingPhotoId}`, {
        method: 'PUT',
        body: JSON.stringify({ ...photoForm, restaurantSlug }),
      });
      setPhotoForm({ title: '', description: '', imageUrl: '', categoryId: '' });
  ...
  async function deleteCategory(category) {
    if (!writable) return;
    if (!window.confirm(`Delete gallery category "${category.name}" and its photos?`)) return;
    try {
    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <form onSubmit={editingCategoryId ? updateCategory : submitCategory} className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
          <h2 className="text-lg font-semibold">{editingCategoryId ? 'Edit gallery category' : 'Add gallery category'}</h2>
        ...
        <form onSubmit={editingPhotoId ? updatePhoto : submitPhoto} className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
          <h2 className="text-lg font-semibold">{editingPhotoId ? 'Edit photo' : 'Add photo'}</h2>
          <div className="mt-4 grid gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              <button disabled={!writable} className="rounded-md bg-[#10241f] px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">{editingPhotoId ? 'Update photo' : 'Save photo'}</button>
              {editingPhotoId ? <button type="button" onClick={resetPhotoForm} className="rounded-md border border-neutral-200 px-4 py-2 text-sm font-semibold text-neutral-700">Cancel edit</button> : null}
            </div>
          </div>
        </form>
      </section>
          ...
          {photos.length ? photos.map((photo) => (
            <div key={photo.id} className="rounded-md border border-neutral-100 p-3">
              ...
              <div className="mt-3 flex items-start justify-between gap-3">
                ...
                <div className="flex gap-3">
                  <button disabled={!writable} onClick={() => startEditPhoto(photo)} className="text-sm font-semibold text-emerald-800 disabled:cursor-not-allowed disabled:opacity-40">Edit</button>
