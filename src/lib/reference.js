...
/**
 * Generate a predictable, timestamp-based purchase request reference string.
 * Format: AD-PR-YYMMDDHHmmss-msssRRRR
 * @returns {string} Unique purchase request reference identifier.
 */
export function generatePurchaseRequestReference() {
  const now = new Date();
  const pad = (num, size) => num.toString().padStart(size, "0");
  ...
  return `AD-PR-${YY}${MM}${DD}${HH}${mm}${ss}-${ms}${rand}`;
}
