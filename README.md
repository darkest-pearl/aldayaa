...
- Tenant purchase invoice payment recording foundation added.
- Restaurant staff can record tenant-scoped manual payment records against purchase invoices under `/r/[restaurantSlug]/admin/purchase-invoices`.
...
- Tenant audit logging and security hardening foundation added.
...
- Tenant payment settings foundation added.
- The tenant payment settings foundation stores disabled/planned-only provider readiness fields in a focused nullable `paymentSettings` field on RestaurantSettings because existing display/settings fields were not safe to overload.
- Payment settings do no real payment processing, make no provider calls, and keep no secrets stored in database records.
- Payment settings do not add refunds, checkout sessions, provider webhooks, subscription charging, provider SDKs, packages, seed data, or production database writes.
- Payment webhook idempotency foundation added.
- The payment webhook idempotency foundation adds provider-neutral `PaymentProviderEvent` records for future webhook idempotency and read-only tenant visibility under `/r/[restaurantSlug]/admin/payment-provider-events`.
- OWNER and MANAGER can view scoped sanitized payment provider events; SUPPORT cannot access this view.
- For Batch 75, no webhook endpoint is active, no real payment processing occurs, no provider calls are made, and no raw card data or secrets stored.
